...
def show_start_screen():
    ...
    try:
        # Muat gambar background untuk layar awal
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    running = False # Keluar dari loop jika spasi ditekan
        ...
        if background_img:
            ...
            screen.fill(BLACK) # Fallback jika gambar tidak ada
...
def show_lobby_screen(client_interface):
    try:
        # Muat background untuk layar pilih karakter
    ...
    while True:
        ...
        if state['status'] != 'OK':
            ...
            b_text = font_medium.render("Tekan [B] untuk Tim Anjing", True, GREY if b_taken else WHITE)
            w_text = font_medium.render("Tekan [W] untuk Tim Kucing", True, GREY if w_taken else WHITE)
            
            if my_pid:
                wait_text = font_medium.render("Menunggu Lawan...", True, GREEN)
                screen.blit(wait_text, wait_text.get_rect(center=(WIDTH // 2, HEIGHT // 2)))
            else:
                 # Tampilkan pilihan hanya jika pemain belum memilih
...
def main_game_loop():
    ...
    # --- PERUBAHAN: Muat gambar custom untuk item ---
    try:
    ...
    while running:
        ...
        if match_ended:
            ...
            status_text = font_medium.render(msg, True, color)
            ...
            # --- PERUBAHAN: Gunakan gambar custom saat membuat Gem ---
            for g in state['gems']:
                if g['id'] not in gem_objects:
                    # Buat objek Gem seperti biasa (menggunakan gambar B64 dari server sebagai fallback)
                    new_gem = Gem(g['id'], g['x'], g['y'], g['type'], images_b64.get(f"{g['type']}_gem"))
                    
                    # Timpa gambar dengan gambar lokal yang sesuai
            ...
            # --- PERUBAHAN: Ubah teks "Gems" menjadi "Treats" ---
            ...
            if game_info['stage_winner']:
                ...
                winner_name = "Dog (HITAM)" if "black" in game_info['stage_winner'] else "Cat (PUTIH)"
                stage_win_text = font_small.render(f"TIM {winner_name} MENANG STAGE INI!", True, WIN_GREEN)
